...
#   print(len_s,bs,self_attn_inputs.shape,"____________________len_s,bs,self_attn_inputs___________")
...
class TemporalFusionTransformer(object):
  ...
  def get_tft_embeddings(self, all_inputs):
    ...
    # print(obs_inputs[0].shape,"+++++++++++++++++++++++++++++++++obs_input")
    obs_inputs = tf.keras.backend.stack(obs_inputs,axis=-1)
    # print(obs_inputs.shape,"+++++++++++++++++++++++++++++++++obs_input")
  ...
  def _build_base_graph(self):
    ...
    print(f"known_combined_layer shape: {known_combined_layer[:, :encoder_steps, :].shape}")
    print(f"obs_inputs shape: {obs_inputs[:, :encoder_steps, :].shape}")
    ...
    #   print(type(embedding.shape),embedding.shape,"___________________________________embedding.shape_______________")
    ...
    #   print(type(sparse_weights),sparse_weights.shape,"___________________________________sparse_weights.shape_______________")
      trans_emb_list = []
    ...
    #   print(type(transformed_embedding), transformed_embedding.shape,"___________________________________transformed_embedding.shape_______________")
      combined = tf.keras.layers.Multiply()(
          [sparse_weights, transformed_embedding])
    #   print(type(combined), combined.shape,"___________________________________combined.shape_______________")
    ...
    # print(historical_features.shape, future_features.shape,"________historical_features, future_features")
    ...
    # print(type(enriched),enriched.shape,"___________enrichned_______")
    mask = get_decoder_mask(enriched)
    
    # print(mask.shape,"______________________mask________________")
    x, self_att \
    ...
    # print(decoder.shape,"__________decoder.shape________________________")
    transformer_layer = add_and_norm([decoder, temporal_feature_layer])
    # print(transformer_layer.shape,"__________transformer.shape________________________")
    ...
    print(transformer_layer.shape, all_inputs.shape,"___________transformer_layer, all_inputs, attention_components__________________")
  ...
  def build_model(self):
    ...
    with tf.compat.v1.variable_scope(self.name):
      ...
      outputs = tf.keras.layers.TimeDistributed(
          ...
          tf.keras.layers.Dense(self.output_size ))(transformer_layer[Ellipsis, self.num_encoder_steps:, :]) # Chnaged line
      ...
      model.compile(
          loss=tf.keras.losses.MSLE, optimizer=adam, sample_weight_mode='temporal')
  ...
  def fit(self, train_df=None, valid_df=None):
    ...
    print(np.array(labels).shape,data.shape,"____y,x shape_____")
    
    # print(np.array(labels)[1:2],'labels')
    val_data, val_labels, val_flags = _unpack(valid_data)
    # print("______process_id__", multiprocessing.current_process().pid)
    all_callbacks = callbacks
    self.model.fit(
        x=data,
        y=np.array(labels),#np.concatenate([labels, labels, labels], axis=-1),
        sample_weight=active_flags,
        epochs=self.num_epochs,
        batch_size=self.minibatch_size,
        validation_data=(val_data,val_labels),# np.concatenate([val_labels, val_labels, val_labels], axis=-1), val_flags),
  ...
  def get_hyperparm_choices(cls):
    ...
    return {
        'dropout_rate': [0.2,0.23,0.27,0.3],
        'hidden_layer_size': [30,35,40,50,60,70,80],
        'minibatch_size': [1024],
        'learning_rate': [5*10**-3,5*10**-4,5*10**-5,10**-6,5*10**-7],
        'max_gradient_norm': [50.0,75.0,100.0,125.0,150.0],
        'num_heads': [4,8,12,16],
        'stack_size': [1,5,10],
    }